...
import { WORKSHOP, COACH, type TerminView } from "@/lib/config";

/**
 * Obsah thank-you stránky pro KONKRÉTNÍ termín.
 * Používají ho obě routy: /dekujeme/[slug] i fallback /dekujeme.
 */
export function DekujemeObsah({ termin }: { termin: TerminView }) {
  const mailSubject = encodeURIComponent(`Workshop ${termin.dateShort} - dotaz`);
  ...
  return (
    <main className="min-h-screen bg-cream">
      {/* Konverze pro Meta Pixel */}
      <MetaPixelPurchase value={termin.priceNumber} terminId={termin.id} />
      ...
      <section className="container-x py-12 sm:py-14 max-w-2xl mx-auto space-y-5 sm:space-y-6">
        ...
        <article className="bg-white rounded-2xl shadow-soft p-6 sm:p-8 border-t-4 border-teal-400">
          ...
          <div className="mt-5 flex items-center gap-3 p-3 sm:p-4 bg-cream rounded-xl border border-navy-100/40">
            ...
            <CopyButton text={termin.zoomUrl} label="Kopírovat" copiedLabel="Zkopírováno" />
          </div>

          <div
            className={`mt-4 grid grid-cols-1 ${termin.zoomPassword ? "sm:grid-cols-2" : ""} gap-3 text-sm`}
          >
            <div className="flex items-center justify-between p-3 bg-cream rounded-lg border border-navy-100/40">
              ...
              <span className="font-semibold text-navy-700 font-mono">{termin.zoomId}</span>
            </div>
            {termin.zoomPassword && (
              <div className="flex items-center justify-between p-3 bg-cream rounded-lg border border-navy-100/40">
                <span className="text-dark/60">Heslo:</span>
                <span className="font-semibold text-navy-700 font-mono">{termin.zoomPassword}</span>
              </div>
            )}
          </div>
        </article>
        ...
        <article className="bg-white rounded-2xl shadow-soft p-6 sm:p-8 border-t-4 border-navy-600">
          ...
          <p className="text-sm sm:text-base text-dark/70 mb-5 leading-relaxed">
            Workshop je intenzivní {termin.duration} živá praxe. Krátká příprava zvedne hodnotu
            výrazně.
          </p>
            ...
            {PREP_STEPS.map((step, idx) => (
              <li key={idx} className="flex gap-3 sm:gap-4">
                ...
                <div className="flex-1 pt-0.5">
                  <p className="font-semibold text-navy-700 text-base leading-tight mb-1">
                    {step.title}
                  </p>
        ...
        <article className="bg-navy-50 rounded-2xl p-6 sm:p-8 border border-navy-100">
          ...
          <p className="text-sm sm:text-base text-dark/75 leading-relaxed">
            ...
            - najdeme řešení. Přesun na další termín, vrácení peněz nebo cokoliv jiného. Bez
            papírování, bez stresu.
          </p>
          <p className="text-sm text-dark/60 mt-3">- {COACH.fullName}</p>
        </article>
        ...
        <div className="text-center pt-6 pb-2">
          ...
          <p className="mt-3 text-xs sm:text-sm text-dark/55 italic">
            5 minut před začátkem - start workshopu v {termin.timeStart}
          </p>
        </div>
